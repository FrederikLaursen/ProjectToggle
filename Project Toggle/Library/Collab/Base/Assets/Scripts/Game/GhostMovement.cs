    ...
    void Update()
    {
        ...
        for (int i = 0; i < times.Count; i++)
        {
            if (Time.timeSinceLevelLoad >= times[i].time - Time.deltaTime * 4 && !times[i].hasBeenReached)
            {
                ...
                if(Vector2.Distance(transform.position, times[1].position) > 3)
                {
                    Debug.Log(Vector2.Distance(transform.position, times[1].position));
                    transform.position = times[1].position;
                }
            }
        }
    }
